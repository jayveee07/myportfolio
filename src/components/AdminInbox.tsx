...
import { ADMIN_EMAIL, ADMIN_NAME } from '../lib/supabase';
...
export const AdminInbox = ({ user }: { user: { email?: string } }) => {
  const [conversations, setConversations] = useState<Conversation[]>([]);
  const [archivedConversations, setArchivedConversations] = useState<Conversation[]>([]);
  const [selectedConvoId, setSelectedConvoId] = useState<string | null>(null);
  const [messages, setMessages] = useState<Message[]>([]);
  const [visits, setVisits] = useState<Visit[]>([]);
  const [activeTab, setActiveTab] = useState<'messages' | 'visits' | 'archived'>('messages');
  ...
  const selectedConvo = conversations.find(c => c.id === selectedConvoId) || archivedConversations.find(c => c.id === selectedConvoId) || null;
    ...
    if (user?.email === ADMIN_EMAIL) {
      ...
      (async () => {
        ...
        const { data: rawVisits } = await supabase
          ...
          .limit(100);
        if (rawVisits) {
          setVisits(rawVisits.map((v: Record<string, unknown>) => ({
            id: v.id as string,
  ...
  const handleInspectVisitor = async (email: string) => {
    ...
    const { data: rawData } = await supabase
      ...
      .eq('email', email)
      .order('timestamp', { ascending: false });
    const history: Visit[] = (rawData || []).map((v: Record<string, unknown>) => ({
      id: v.id as string,
      visitorId: v.visitor_id as string,
      email: v.email as string | undefined,
      userAgent: v.user_agent as string,
  ...
  const handleDeleteConversation = (e: React.MouseEvent, convo: Conversation) => {
    e.stopPropagation();
    setConfirmModal({
      type: 'delete',
      title: 'Archive Conversation?',
      message: `This will archive the conversation with ${convo.visitorName || 'this visitor'} and all its messages. The visitor can resume it by sending a new message.`,
      onConfirm: async () => {
        try {
          await deleteConversation(convo.id);
          if (selectedConvoId === convo.id) setSelectedConvoId(null);
          setConfirmModal(null);
        } catch (err: any) {
          setError(err.message);
          setConfirmModal(null);
  ...
  const handleHardDeleteConversation = (e: React.MouseEvent, convo: Conversation) => {
    e.stopPropagation();
    setConfirmModal({
      type: 'delete',
      title: 'Permanently Delete?',
      message: `This will permanently delete the conversation with ${convo.visitorName || 'this visitor'} and all its messages from the database. This cannot be undone or recovered.`,
      onConfirm: async () => {
        try {
          await hardDeleteConversation(convo.id);
  ...
  return (
    <div className="flex h-full bg-surface-alt overflow-hidden font-sans">
      {/* Sidebar - Conversation List */}
      <aside className={`w-full md:w-72 bg-surface border-r border-border flex flex-col transition-all duration-300 ${selectedConvo ? 'hidden md:flex' : 'flex'}`}>
        <div className="p-6 border-b border-border bg-surface/50 backdrop-blur-md sticky top-0 z-20">
          ...
          <div className="flex gap-1 bg-surface-alt/80 p-1 rounded-2xl mb-4 border border-border/50">
            <button 
              onClick={() => setActiveTab('messages')}
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-[10px] font-black uppercase tracking-[0.1em] rounded-xl transition-all ${
                activeTab === 'messages' ? 'bg-surface text-primary shadow-md shadow-border/50 ring-1 ring-border' : 'text-secondary hover:text-primary'
              }`}
            >
            ...
            <button 
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-[10px] font-black uppercase tracking-[0.1em] rounded-xl transition-all ${
                activeTab === 'archived' ? 'bg-surface text-primary shadow-md shadow-border/50 ring-1 ring-border' : 'text-secondary hover:text-primary'
              }`}
            >
            ...
            <button 
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-[10px] font-black uppercase tracking-[0.1em] rounded-xl transition-all ${
                activeTab === 'visits' ? 'bg-surface text-primary shadow-md shadow-border/50 ring-1 ring-border' : 'text-secondary hover:text-primary'
              }`}
            >
              <Eye size={13} /> Visits
            </button>
          ...
          <div className="relative group">
            ...
            <input 
              type="text" 
              placeholder={activeTab === 'messages' ? "Find conversation..." : activeTab === 'archived' ? "Search archived..." : "Lookup visit..."}
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full bg-surface-alt border-2 border-transparent rounded-2xl pl-12 pr-4 py-4 text-sm focus:bg-surface focus:border-accent/10 focus:ring-0 focus:outline-none font-bold placeholder:text-muted transition-all shadow-inner"
            />
          </div>
          ...
          {activeTab === 'messages' ? (
            filteredConversations.length === 0 ? (
              <div className="p-12 text-center">
                <div className="w-12 h-12 bg-surface-alt text-muted rounded-full flex items-center justify-center mx-auto mb-4">
                  <MessageSquare size={24} />
                </div>
                <p className="text-sm font-bold text-secondary uppercase tracking-widest">No messages yet</p>
              </div>
            ) : (
              filteredConversations.map(convo => (
                <button
                  ...
                  className={`w-full p-8 flex flex-col items-start border-l-[6px] transition-all relative group/item border-b border-border/50 ${
                    selectedConvoId === convo.id 
                      ? 'border-accent bg-accent/5' 
                      : (convo.unreadCount || 0) > 0 
                        ? 'border-blue-400/30 bg-blue-50/20' 
                        : 'border-transparent hover:bg-surface-alt/80 bg-surface'
                  }`}
                >
                  <div className="flex items-center justify-between w-full mb-3 relative">
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        ...
                        selectedConvoId === convo.id ? 'bg-accent text-white shadow-lg shadow-accent/20' : 'bg-surface-alt text-secondary'
                      }`}>
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`font-black text-sm block leading-none ${convo.unreadCount ? 'text-primary' : 'text-primary'}`}>
                            {convo.visitorName || 'Guest Visitor'}
                          </span>
                          {convo.isPinned && <Pin size={10} className="text-accent fill-accent shrink-0" />}
                        </div>
                        <span className="text-[10px] text-secondary font-bold uppercase tracking-tight flex items-center gap-1.5 leading-none">
                          <Clock size={10} />
                    ...
                    <span
                      onClick={(e) => handleDeleteConversation(e, convo)}
                      className="opacity-0 group-hover/item:opacity-100 transition-all p-1.5 rounded-lg text-muted hover:text-red-500 hover:bg-red-50 absolute right-2 top-1/2 -translate-y-1/2 cursor-pointer"
                      title="Delete conversation"
                    >
                      <Trash2 size={14} />
                    </span>
                  </div>
                  <p className={`text-xs leading-relaxed line-clamp-1 ml-[52px] ${convo.unreadCount ? 'text-primary font-extrabold' : 'text-secondary font-medium'}`}>
                    {convo.lastMessage && !['Message deleted', 'This message was deleted'].includes(convo.lastMessage) ? convo.lastMessage : 'No recent activity'}
                  </p>
                </button>
              ))
            )
            ...
            archivedConversations.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <p className="text-sm font-bold text-secondary uppercase tracking-widest">No archived conversations</p>
              </div>
            ) : (
              archivedConversations.filter(c =>
                ...
                <button
                  key={convo.id}
                  onClick={() => setSelectedConvoId(convo.id)}
                  className={`w-full p-8 flex flex-col items-start border-l-[6px] border-amber-400/30 transition-all relative group/item border-b border-border/50 ${
                    selectedConvoId === convo.id
                ...
                >
                  <div className="flex items-center justify-between w-full mb-3 relative">
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        <div className={`w-10 h-10 rounded-2xl flex items-center justify-center font-black text-sm transition-all group-hover/item:scale-110 ${
                          selectedConvoId === convo.id ? 'bg-amber-500 text-white shadow-lg shadow-amber-500/20' : 'bg-amber-50 text-amber-600'
                        }`}>
                          <Archive size={16} />
                        </div>
                      </div>
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span className="font-black text-sm block leading-none text-muted">
                            {convo.visitorName || 'Guest Visitor'}
                          </span>
                        </div>
                        <span className="text-[10px] text-secondary font-bold uppercase tracking-tight flex items-center gap-1.5 leading-none">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        onClick={(e) => handleHardDeleteConversation(e, convo)}
                        className="opacity-0 group-hover/item:opacity-100 transition-all p-1.5 rounded-lg text-muted hover:text-red-500 hover:bg-red-50 cursor-pointer"
                        title="Permanently delete"
                      >
                        <Trash2 size={14} />
                      </span>
                  ...
                  <p className="text-xs leading-relaxed line-clamp-1 ml-[52px] text-muted font-medium">
            ...
            <div className="divide-y divide-border">
              {visits.filter(v => 
                ...
                <div key={visit.id} className="p-6 hover:bg-surface-alt transition-colors">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-[10px] text-secondary font-bold uppercase tracking-widest leading-none mb-1">Session ID</p>
                  ...
                  {visit.email && (
                    <div className="flex items-center gap-2 mb-2 p-2 bg-surface-alt rounded-lg border border-border">
                      <Mail size={12} className="text-accent" />
                      <span className="text-[11px] font-medium text-secondary">{visit.email}</span>
                    </div>
                  )}
                  <div className="flex items-center justify-between">
                    <span className="bg-surface-alt text-secondary px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-widest">
                      Path: {visit.path}
                    </span>
      ...
      <main className={`flex-1 flex flex-col bg-surface overflow-hidden transition-all duration-300 ${!selectedConvo ? 'hidden md:flex' : 'flex'}`}>
        {selectedConvo ? (
          <>
            {/* Chat Header */}
            <div className="px-6 py-4 border-b border-border flex items-center justify-between bg-surface/80 backdrop-blur-xl shrink-0 z-20 shadow-sm shadow-border/50">
              <div className="flex items-center gap-4">
                <button 
                  onClick={() => setSelectedConvoId(null)}
                  className="md:hidden p-2 bg-surface-alt hover:bg-surface-alt/80 rounded-xl text-secondary transition-all active:scale-90"
                >
                  <ArrowLeft size={18} />
                </button>
                <div className="relative group/avatar">
                  <div className="w-10 h-10 bg-gradient-to-br from-primary to-slate-700 text-white rounded-xl flex items-center justify-center font-black text-base shadow-lg shadow-primary/20 transition-transform group-hover/avatar:scale-105">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      <Mail size={12} className="text-muted group-hover/mail:text-accent" />
                    ...
                    <span className="flex items-center gap-2 text-[9px] px-2 py-1 bg-surface-alt/80 text-emerald-600 font-black rounded-lg uppercase tracking-widest border border-emerald-100">
                    ...
                    <button 
                      ...
                      className={`p-3 rounded-2xl transition-all active:scale-95 border ${
                        conversationMenuId === selectedConvo.id 
                          ? 'bg-btn text-white border-primary shadow-lg shadow-primary/20' 
                          : 'hover:bg-surface-alt text-muted hover:text-primary border-transparent hover:border-border'
                      }`}
                    >
                    ...
                    <AnimatePresence>
                      {conversationMenuId === selectedConvo.id && (
                        <motion.div
                          ...
                          className="absolute right-0 top-full mt-3 w-56 bg-surface rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-border overflow-hidden z-[60] p-2"
                        >
                          <div className="px-4 py-3 border-b border-border mb-1">
                            <span className="text-[10px] font-black text-secondary uppercase tracking-[0.2em]">Context Actions</span>
                          </div>
                          
                          <button 
                            onClick={(e) => handleTogglePin(e, selectedConvo)}
                            className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-surface-alt transition-colors text-primary group"
                          >
                            <Pin size={16} className={selectedConvo.isPinned ? 'text-accent fill-accent' : 'text-muted group-hover:text-secondary'} />
                          ...
                          <button 
                            ...
                            className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-surface-alt transition-colors text-primary group"
                          >
                            <Download size={16} className="text-muted group-hover:text-secondary" />
                          ...
                          <button 
                            ...
                            className={`w-full flex items-center gap-3 p-3 rounded-xl transition-colors group ${
                              selectedConvo.isBlocked ? 'bg-red-50 text-red-600' : 'hover:bg-red-50 text-primary hover:text-red-600'
                            }`}
                          >
                            <Ban size={16} className={selectedConvo.isBlocked ? 'text-red-500' : 'text-muted group-hover:text-red-500'} />
            ...
            <div 
              ref={scrollRef}
              className="flex-1 overflow-y-auto p-6 space-y-6 bg-surface-alt"
            >
              <div className="flex justify-center mb-4">
                <div className="flex flex-col items-center gap-3">
                  <div className="h-px w-24 bg-border" />
                  <span className="px-5 py-2 bg-surface border border-border rounded-2xl text-[10px] text-secondary font-black uppercase tracking-[0.2em] shadow-sm">
              ...
              {messages
                    .filter(m => !m.deletedBy?.includes(ADMIN_EMAIL.toLowerCase()))
                .map((msg, i) => (
                <motion.div 
                  key={msg.id || i} 
                ...
                >
                  <div className={`flex flex-col ${msg.senderId === ADMIN_EMAIL ? 'items-end' : 'items-start'} max-w-[75%]`}>
                    {editingMessageId === msg.id ? (
                      <div className="bg-white border-2 border-accent p-4 rounded-3xl flex flex-col gap-4 w-full shadow-2xl min-w-[280px]">
                        <textarea
                          ...
                          className="w-full bg-surface-alt border-none rounded-2xl px-5 py-4 text-sm focus:ring-2 focus:ring-accent/10 font-bold resize-none min-h-[100px] shadow-inner"
                        />
                        <div className="flex justify-end gap-3">
                          <button 
                            onClick={handleDiscardEdit}
                            className="p-1 px-5 text-[11px] font-black text-secondary uppercase tracking-widest hover:text-primary transition-colors"
                          >
                            Discard
                          </button>
                          <button 
                      ...
                      <div className={`relative px-6 py-4 rounded-[1.75rem] text-[13px] font-bold leading-[1.6] shadow-sm transition-all group-hover/msg:shadow-md ${
                        msg.isDeleted ? 'bg-surface-alt text-secondary italic font-medium' :
                        msg.senderId === ADMIN_EMAIL 
                          ? 'bg-btn text-white rounded-tr-[0.25rem] shadow-xl shadow-primary/5' 
                          : 'bg-surface text-primary rounded-tl-[0.25rem] border border-border'
                      }`}>
                          ...
                          <div className={`absolute top-1/2 -translate-y-1/2 flex gap-2 opacity-0 group-hover/msg:opacity-100 transition-all duration-300 z-20 ${
                            ...
                            {msg.senderId === ADMIN_EMAIL && (
                              <button 
                                ...
                                className="w-10 h-10 bg-surface text-secondary rounded-xl border border-border shadow-xl flex items-center justify-center hover:text-accent hover:scale-110 active:scale-90 transition-all"
                                title="Edit Record"
                              >
                                <Pencil size={16} />
                              </button>
                            ...
                            <button 
                              ...
                              className="w-10 h-10 bg-surface text-secondary rounded-xl border border-border shadow-xl flex items-center justify-center hover:text-red-500 hover:scale-110 active:scale-90 transition-all"
                              title="Delete Record"
                            >
                              <Trash2 size={16} />
                            </button>
                        ...
                        <AnimatePresence>
                          {deleteOptionsId === msg.id && (
                            <motion.div
                              ...
                              className={`absolute z-30 bg-surface border border-border shadow-[0_20px_50px_rgba(0,0,0,0.15)] rounded-2xl p-2.5 flex flex-col gap-1 min-w-[200px] ${
                                msg.senderId === ADMIN_EMAIL ? 'right-0 top-full mt-4' : 'left-0 top-full mt-4'
                              }`}
                            >
                              <p className="text-[10px] font-black text-secondary uppercase tracking-[0.2em] px-4 py-2 border-b border-border mb-1">Administrative Action</p>
                              {msg.senderId === ADMIN_EMAIL && (
                                <button
                              ...
                              <button
                                ...
                                className="w-full text-left px-4 py-3 text-xs font-black text-primary hover:bg-surface-alt rounded-xl transition-all flex items-center gap-3 group/opt"
                              >
                                <div className="w-1.5 h-1.5 bg-muted rounded-full group-hover/opt:scale-150 transition-transform" />
                                Archive (Private)
                              </button>
                              <button
                                onClick={() => setDeleteOptionsId(null)}
                                className="w-full text-center py-2.5 text-[10px] font-black text-secondary uppercase tracking-widest mt-1 hover:text-primary transition-colors"
                              >
                                Abort
                              </button>
                            </motion.div>
                    ...
                    <div className={`mt-3 flex items-center gap-3 ${msg.senderId === ADMIN_EMAIL ? 'justify-end' : 'justify-start'}`}>
                      {msg.isEdited && (
                        <span className="text-[9px] text-muted font-extrabold uppercase italic tracking-widest bg-surface-alt px-2 py-0.5 rounded-lg border border-border">MODIFIED</span>
                      )}
                      ...
                      {msg.senderId === ADMIN_EMAIL && (
                        <div className="flex items-center gap-0.5">
                           <CheckCheck size={14} className={msg.isRead ? "text-accent" : "text-muted"} />
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <div className="p-6 bg-surface border-t border-border flex flex-col gap-6 shrink-0 shadow-[0_-10px_40px_rgba(0,0,0,0.02)]">
              <AnimatePresence>
                {aiSuggestion && (
                  <motion.div 
                  ...
                  >
                    ...
                    className="p-1.5 hover:bg-surface-alt rounded-xl text-secondary hover:text-primary transition-colors"
                      >
                        <CloseX size={16} />
                      </button>
                    </div>
                    <p className="text-sm text-primary font-bold leading-[1.7] mb-6 relative z-10 pl-4 border-l-4 border-accent/20">
                      {aiSuggestion}
                    </p>
              ...
              <motion.div 
                ...
                className="bg-red-50 text-red-500 text-[10px] font-black p-4 rounded-2xl border border-red-200 flex items-center gap-4 mb-2 shadow-sm"
                  >
              ...
              <form onSubmit={handleSend} className="flex gap-4 max-w-5xl mx-auto items-center w-full px-2">
                <div className="flex-1 relative flex items-center group">
                  <input
                    ...
                    className="w-full bg-surface-alt border-2 border-transparent rounded-[1.5rem] px-8 py-5 pr-14 text-sm focus:bg-surface focus:border-accent/10 focus:ring-0 focus:outline-none font-bold placeholder:text-muted transition-all shadow-inner"
                  />
                  <button
                    type="button"
                    onClick={handleGetAiSuggestion}
                    ...
                    className={`absolute right-4 p-2.5 rounded-xl transition-all ${
                      isAiLoading ? 'text-accent animate-spin' : 'text-muted hover:text-accent hover:bg-accent/5'
                    }`}
                ...
                <button
                  type="submit"
                  disabled={!input.trim()}
                  className="h-[64px] px-10 bg-btn text-white rounded-[1.5rem] font-black uppercase tracking-widest hover:bg-slate-800 disabled:opacity-20 disabled:cursor-not-allowed transition-all flex items-center gap-3 shadow-2xl shadow-primary/20 hover:shadow-primary/30 active:scale-[0.98] active:shadow-inner shrink-0"
                >
                  Send <Send size={20} />
                </button>
              </form>
              <div className="flex justify-center text-[10px] font-black text-muted uppercase tracking-[0.3em]">
                Secure Administrative Session
              </div>
            </div>
          </>
        ) : (
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center bg-surface-alt/30">
            <div className="w-24 h-24 bg-surface rounded-3xl shadow-xl flex items-center justify-center mb-8 rotate-3">
      ...
      <AnimatePresence>
        {inspectingVisitor && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="p-8 border-b border-border flex items-center justify-between bg-surface z-10 sticky top-0">
                <div>
                ...
                <button 
                  onClick={() => setInspectingVisitor(null)}
                  className="p-2 hover:bg-surface-alt rounded-xl text-secondary"
                >
                  <CloseX size={24} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-8">
                <div className="space-y-4">
                  {inspectingVisitor.history.map((visit) => (
                    <div key={visit.id} className="p-4 bg-surface-alt rounded-2xl border border-border">
                      ...
                      <p className="text-xs text-secondary font-medium leading-relaxed">
                         <span className="font-bold text-primary uppercase text-[9px] tracking-tight block mb-1">User Agent:</span>
                         {visit.userAgent}
                      </p>
                    </div>
                  ))}
                  {inspectingVisitor.history.length === 0 && (
                    <div className="text-center py-12">
                      ...
                      <p className="text-secondary font-medium">No history found for this email.</p>
                    </div>
                  )}
                </div>
              </div>
        ...
        {confirmModal && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-sm bg-surface rounded-[2rem] shadow-2xl overflow-hidden p-8 flex flex-col items-center text-center border border-border"
            >
              ...
              <div className="flex gap-3 w-full">
                <button 
                  onClick={() => setConfirmModal(null)}
                  className="flex-1 py-4 px-6 bg-surface-alt hover:bg-surface-alt/80 text-secondary rounded-2xl text-xs font-black uppercase tracking-widest transition-all active:scale-95 border border-border/50"
                >
                  Cancel
                </button>
                <button 
